...
export async function processAction(env, action) {
  ...
  // A proof-based draft is local, deterministic, and has no external side effect.
  // Keep a configured generator webhook as an opt-in override, but do not make the
  // basic content workflow depend on a third-party model or credential.
  if (action.action_type === "generate_content" && !env.generatorWebhookUrl) {
    const result = buildProofDraft(campaign, content);
    await patchGeneratedContent(env, action, content, result);
    await finishAction(env, action.id, "succeeded", { result, last_error: null });
    return { id: action.id, status: "succeeded", native_draft: true };
  }
...
export function buildProofDraft(campaign, content) {
  const proof = removeBlockedClaims(redactSensitiveText(content?.proof_summary), campaign?.blocked_claims);
  if (!proof) throw new Error("Proof summary is empty after privacy and claim checks.");

  const service = redactSensitiveText(content?.service || content?.title || "completed repair");
  const city = redactSensitiveText(content?.city || "");
  const heading = [service, city ? `in ${city}` : ""].filter(Boolean).join(" ").trim();
  const callToAction = redactSensitiveText(content?.call_to_action || campaign?.call_to_action || "Request a repair at HomeRepairSLC.com.");
  const cleanHeading = removeBlockedClaims(heading, campaign?.blocked_claims) || "Home Repair SLC project update";
  const cleanCta = removeBlockedClaims(callToAction, campaign?.blocked_claims) || "Request a repair at HomeRepairSLC.com.";

  return {
    ok: true,
    generator: "native_proof_draft_v1",
    draft_copy: `${cleanHeading}\n\n${proof}\n\n${cleanCta}`.slice(0, 5000),
    proof_summary: proof.slice(0, 2000),
    call_to_action: cleanCta.slice(0, 1000),
  };
}

async function patchGeneratedContent(env, action, content, result) {
  ...
  await patchRows(env, "marketing_content", {
    ...
    notes: [content.notes, `Draft generated by action ${action.id}; human or envelope approval is still required.`].filter(Boolean).join("\n"),
  }, { id: `eq.${content.id}`, user_id: `eq.${env.userId}` });
}
...
function redactSensitiveText(value) {
  return String(value || "")
    ...
    .replace(/\b\d{1,6}\s+[A-Za-z0-9.'-]+(?:\s+[A-Za-z0-9.'-]+){0,4}\s+(?:street|st|avenue|ave|road|rd|drive|dr|lane|ln|court|ct|boulevard|blvd|place|pl|way)\b/gi, "[address removed]")
    .replace(/\s+/g, " ")
    .trim();
}

function removeBlockedClaims(value, blockedClaims) {
  const blocked = Array.isArray(blockedClaims)
    ? blockedClaims
    : String(blockedClaims || "").split(/[\n,]/);
  const phrases = [...blocked, "licensed", "insured", "bonded", "guaranteed", "guarantee", "warranty", "best", "top-rated", "same-day", "24/7"]
    .map((item) => String(item || "").trim().toLowerCase())
    .filter(Boolean);
  return String(value || "")
    .split(/(?<=[.!?])\s+/)
    .filter((sentence) => !phrases.some((phrase) => sentence.toLowerCase().includes(phrase)))
    .join(" ")
    .trim();
}
...
export function executionModeForAction(action, env) {
  if (action.action_type === "generate_content" && !env.generatorWebhookUrl) return "live";
